...
public class WechatDecriptUtil {
    ...
    public static JSONObject getDecryptedData(String sessionKey, String encryptedData, String iv) {
        ...
        if (null != sessionKey && null != encryptedData && null != iv) {
            byte[] sessionKeyBy = Base64.decode(sessionKey.getBytes());
            byte[] encryptedDataBy = Base64.decode(encryptedData.getBytes());
            byte[] ivBy = Base64.decode(iv.getBytes());
